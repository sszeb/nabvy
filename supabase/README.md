...
the download page size). `20260924030000_apify_gateway_input_hardening.sql` (task 1.1a) is not
applied yet: the coordinator applies it after merge. Until then the live gateway accepts inputs the
repository refuses, so enqueue only inputs built by `services/source-adapters`.
