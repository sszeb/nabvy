...
1. `git fetch origin claude/coordinator-3` and base `claude/coordinator-4` on it. PR #25 merged 14:53 (`1727607`); coordinator 4 works on `claude/coordinator-4`. Open at 15:40: #28 0.10 (changes needed), #29 apify-gateway (migration plus an Edge Function deploy after merge; see `docs/progress.md`) and #30 (this branch, docs). Round 2 running: product-catalogue, apify-gateway, waitlist, account (IDs in `docs/progress.md`).
