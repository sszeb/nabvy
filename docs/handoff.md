...
3. Subscribe to the open PRs: #29 (apify-gateway: after merge apply its migration, then deploy the Edge Function `supabase/functions/apify-gateway` with `deploy_edge_function`, verify_jwt off; it stays inert until an admin switches on `apify` and `apify-gateway`), #30, and each PR the round-2 sessions open.
...
1. `git fetch origin claude/coordinator-3` and base `claude/coordinator-4` on it. PR #25 merged 14:53 (`1727607`); coordinator 4 works on `claude/coordinator-4`. Open at 15:45: #29 apify-gateway (changes needed 15:41: month-boundary cap, cost-meter pause, merge main; migration plus an Edge Function deploy after merge, see `docs/progress.md`) and #30 (this branch, docs). #28 0.10 merged 15:40. Round 2 running: product-catalogue, apify-gateway, waitlist, account (IDs in `docs/progress.md`).
