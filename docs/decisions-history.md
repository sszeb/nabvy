# Decisions and constraints — full history

This is the complete chronological decision record, verbatim, moved out of `docs/decisions.md` in the 2026-09-26 docs slimming so that file could hold only the rules currently in force. Every heading, quote and superseded reading below is preserved; nothing has been reworded or cut. For the condensed current ruleset, see `docs/decisions.md`.
...
| Cadence and tiers | Tiers are sold by cadence: Standard 5-minute, Pro and Business 1-minute (`docs/decisions.md`, Pricing) | T2 reported on 2026-09-24 (guide commits `23da75a`, `f323632`): per centre, newest-first page-1 checks hourly in active hours are the primary alert path (86–91% of new local broad-term listings, about $2 a month per two-term centre) and the full sweep runs daily by the owner's decision (4–6 h per centre only if nationwide demand appears); newest-first plus sweep reaches about 90% recall and lag cannot beat the check interval. Cadences faster than hourly were not tested; nothing faster than hourly is sold as guaranteed, and no boost or "instant" tier is sold, until the owner answers the 2026-09-25 question in `docs/questions.md` (`HANDOFF.md`, `APP_INTEGRATION_GUIDE.md` §7, §9) *(Updated by coordinator 10 on 2026-09-25 from the guide's T2 commits; backlog 0.18.)* |
...
| Search planning | One watch per marketplace, category and 40 km cell, H3 resolution 4 (`docs/architecture.md`, `docs/engineering.md`) | Per region: a verified centre `cityId` × a few terms, never per user (seed: `city-pages.seed.json`, 771 city IDs, 5 verified centres). Newest-first checks and daily sweeps only — the default-order catch-up job is dropped (T2: 9–38% recall; `APP_INTEGRATION_GUIDE.md` §7 row 2, "Do not schedule it"); the app chooses which IDs get details and sends them as `listingIds` batches; the actor never filters or judges *(Updated by coordinator 10 on 2026-09-25 from the guide's T2 commits; backlog 0.18.)* |
