...
| 0.8 | Config per module | in review | 2026-09-24 | Raised by cost-meter (PR #16). `@nabvy/config` gains a `./modules/*` export and the per-module pattern (Zod-validated, rule 14); `packages/config/src/modules/cost-meter.ts` is the first file, moved from `services/cost-meter/src/config.ts`. `USD_GBP_RATE` moved out of the `apify` group into its own `exchangeRate` group (`docs/questions.md`). |
