...
| 0.10 | Analytics and observability clients | not started | 2026-09-24 | Plan in `docs/design/analytics-growth.md`; waits for nothing (keys optional). |
...
| 4.0b | Auth: audit admin actions | in review | 2026-09-24 | Admin actions (`setRole`, `restrictAccount`, `liftRestriction`, `revokeSessions`, founder promotion) write one `audit_log` row in their own transaction and roll back without it; Better Auth's mutating admin endpoints refused. One migration for the coordinator: `better-auth/20260924143843_better_auth_audit_access.sql` (insert on `audit_log.entries` for `nabvy_auth`) |
