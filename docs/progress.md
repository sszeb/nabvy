...
| 0.2 | Contracts package | in progress | 2026-09-24 | With 0.3, as the shared foundation for atomic modules (per-module contract files, event registry, module scaffold): session on `task/0.2-0.3-foundation`. Started ahead of catalogue approval on the owner's instruction (`docs/decisions.md`, "How modules are built") |
| 0.3 | Database schema | in progress | 2026-09-24 | With 0.2 on `task/0.2-0.3-foundation`: per-module schemas, roles, `withUser` and RLS, migration layout for parallel branches, local dry-run. Seeds for H3 cells dropped (Precedence row "Search planning") |
...
| Actor reference rebuild | in progress | 2026-09-24 | `docs/fb-actor-reference.md` rebuilt from the owner's listed files only, with a report of what was dropped |
