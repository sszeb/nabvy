...
| 1.1 | Apify Facebook adapter | in progress | 2026-09-24 | Groundwork only, while the actor's integration guide is pending: the route-health helper ported from the actor with its tests, plus a pinned test for the description-missing caveat; actor-input validation (the actor's v3 rules plus the gateway's) and the four run presets, sized to the gateway cap (`services/source-adapters/README.md`). The adapter itself waits for the guide and task 0.2 |
