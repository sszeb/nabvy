...
These two are GitHub Actions repository secrets, not application config: they never reach `packages/config` or `.env`. Only the two CI jobs above read them, each to `POST` to `https://api.anthropic.com/v1/claude_code/routines/<id>/fire` with its token, so a CI run on a green or failed head can wake the reviewer or Fixer Routine without a human or a long-lived session in the loop.
