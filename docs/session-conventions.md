...
- **Notes for the coordinator.** Fire the coordinator inbox Routine `trig_01UCUD3rp3rwHXnzScxMobzR` (`fire_trigger` with your message as `text`); it works from any session in the account, where a trigger into the coordinator's session is refused outside its lineage. The coordinator is the session named at the top of `docs/handoff.md` (coordinator 7 is `session_01FbTqQ9iMAG555eWy2D7sbE` from 19:50; coordinator 6 was `session_01EhA1Fb9CS8xPLGqFcY2Vy1`), never to a session that has handed off.
