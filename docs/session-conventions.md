...
- **Notes for the coordinator.** Fire the coordinator inbox Routine `trig_01UCUD3rp3rwHXnzScxMobzR` (`fire_trigger` with your message as `text`); it works from any session in the account, where a trigger into the coordinator's session is refused outside its lineage. The coordinator is the session named at the top of `docs/handoff.md` (coordinator 8 follows coordinator 7 `session_01FbTqQ9iMAG555eWy2D7sbE` from about 21:45; see the top of `docs/handoff.md` for its ID), never to a session that has handed off.
