...
Memory: `reviewed.txt` on branch `claude/coordinator-state`, lines `#<n> <sha7> claimed|approved|merged|changes <UTC>`, written only by `scripts/precheck.mjs`.
...
3. **Claim:** `node scripts/precheck.mjs claim <n> <head>`. `TAKEN` → end in one line (another run has it). `UNCLAIMED` (push refused) → go on only if the head has no review yet.
...
7. **Record:** `node scripts/precheck.mjs record <n> <head> merged|approved|changes` (`approved` = CI still running; a later backstop merges it).
