...
| Hold and clawback | Commissions become payable 30 days after the invoice (the chargeback window; there are no refunds, `docs/decisions.md` "No refunds"); chargebacks and legally required refunds reverse the commission |
...
4. **Chargebacks and legally required refunds.** Their handler calls Dub to reverse the sale, which claws back the commission.
...
Lead tracked once per user; sale tracked once per invoice (idempotent by invoice ID); a chargeback or legally required refund reverses; the £5 credit written only on the first paid invoice; a user with no click ID and no code produces no calls.
