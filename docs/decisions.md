...
2. **Waves.** Every module whose inputs and owner decisions are ready starts at the same time, each in its own session, on its own branch `task/<id>-<module>`, with one pull request per module. Each module is built and tested against fixtures. Migrations are tested only on a local throwaway Postgres (`pnpm db:dry-run`); the reviewer session applies them to Supabase straight after merging the pull request (owner's working-economy request, 2026-09-24; it had just reviewed them, and the coordinator no longer polls). A module waiting on an owner decision or a legal gate waits for a later wave.
