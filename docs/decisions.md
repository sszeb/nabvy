...
- **Beta coverage and Apify budget** (owner, 2026-09-24).
  - **Coverage.** A national UK grid, with Ireland skipped for now. Search centres are Facebook city pages about 80–100 km apart covering Great Britain and Northern Ireland, chosen from `city-pages.seed.json` and each confirmed by a cheap verification run. Prices are GBP only, so no EUR groups are needed yet.
