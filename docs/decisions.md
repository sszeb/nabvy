...
- **Beta coverage and Apify budget** (owner, 2026-09-24).
  ...
  - **The one exception: our own test hunt.** The owner's team tests the app on a real hunt for **"rtx3090"**. Its area is Chichester, the verified centre the actor's own tests used, unless the owner names another. This hunt is the first end-to-end acceptance test of the pipeline: search, ingest, parts and noise filtering, copy-advert detection, asking-price position, and an alert delivered.
