...
5. **A rate limit is not a budget** (G10, G17). 30 scans a minute is 43,200 paid calls a day. Owner (`docs/decisions.md`, "Free-tier limits, paid users and the daily cap"; free tier only): **each free account costs at most £2 a day**, counting every paid action (bursts, scans, pastes, details, model calls), checked in the gate before submit and editable in admin. With a £20 daily pool at most 10 free accounts can run dry; the rest slow down as the pool drains. Paid accounts have no daily cap: their credits and the global caps bound them.
...
| E4 | **Scan abuse**: 30 a minute per user is 43,200 vision calls a day | Rate limit only (engineering) | `scan-lookup`, `scan-recognition`, `usage-ledger` | `scan-quota`: 43,200 scans in a day stop at a free account's £2 daily cap; paid scans are charged | ≤ £2 a free account a day | **G10**, G17 |
...
2. **£2 a day per free account** (G17), and **paid never waits**: sign-ups and upgrades pass a breaker trip or a spent pool (G7).
