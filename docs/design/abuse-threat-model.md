...
From `docs/decisions.md`, "Sign-up throttle and surge stop" (owner, 2026-09-24, 17:08), with the owner's addition on farming and extraction (17:07). Test plan and adversarial pass included.
...
Below: 31 exploits, each with its guard, owner, fixture, run-off bound and gap; then farming and extraction; then gaps, the day-one checklist and the adversarial pass.
...
- Model calls pass the same gate (1.6d); one outside it is a finding.
...
| --- | --- | --- | --- | --- | --- | --- |
| A1 | **Sign-up flood**: 100 accounts in a minute from rotating residential IPs, each firing a free burst in a distinct lone area | Edge (4.3u); 5 sign-ups an hour per IP; identity limits (4.3q); throttle and admission queue (4.3s); breakers (1.6e); pool (1.6c); lifetime cap (4.9a). Planned | `account-integrity`, `spend-governor`, edge | `signup-flood-100`: 100 sign-ups from 100 IPs in 60 s. At most the policy's number are admitted and the rest are queued in order. The breaker trips before a concurrency cap's worth of submits. Free spend ≤ pool + C × R. The run is repeated with 1,000 sign-ups, through both the magic-link route and the Google sign-in callback | ≤ £22 a day (pool + run-off). The 100 accounts' combined £200 lifetime is never reachable in a day | G1 (guards hard from day one) |
...
| D2 | **Retry storm**: a broken actor fails every run; each resubmit spends | Event retries capped (0.9a); run retries unbudgeted | `check-scheduler`, `details-queue`, `spend-governor` | `retry-storm`: a run shape failing 5 times in 10 minutes holds that shape (a failure-rate breaker in 1.6e) and alerts. Each area resubmits at most N times an hour | ≤ breaker threshold × R | **G14** |
...
| D6 | **Gate race**: 50 submits pass the gate together before any reservation is written | The gate checks reserved plus settled (1.6d). Atomicity is not specified | `spend-governor` | `gate-race`: 50 concurrent submits against £1 of headroom book ≤ £1 + R; a per-user variant does the same for concurrent wants against one balance (G9) | ≤ C × R once atomic; unbounded between gate and write without it | **G6** |
...
A critic tried to defeat every guard and to find missing exploits (12 findings). What survived has been folded in:

- **Kept and folded.** OAuth sign-in covered by the flood fixture (A1). The Telegram chat added to the trial keys that survive deletion (A3, G2). Funded-area piggyback: each account still uses up its windows and is charged its attributed share, and a surge of accounts into one area is held (A4). Bulk reads hard for free traffic from launch (E6, G16). The elevated admin session bound to its device (F1, G5). A per-user gate race (D6, G9). Rotated device fingerprints still caught by ASN clustering (`farm-velocity`, 4.3ab). The reservation arithmetic now states that reservations are per check and held until settlement (summary, finding 3).
...
- **Day boundary (new, bounded).** A flood at 23:58 and again at 00:02 would get two days' pools. The first flood trips the breaker, which resets only by an admin, and the weekly and monthly caps still hold. Periods are Europe/London (1.6d). Fixture `day-boundary-double-spend`: floods at 23:59 and 00:01 book ≤ one pool + C × R while the breaker is tripped. Row 1.6g.
