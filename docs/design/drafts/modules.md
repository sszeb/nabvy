...
- **Depends on:** `switches`, `audit-log`, `listing-ingest`, `detail-evidence`, `city-pages`, `location`, `quote-redaction`, `spend-governor`, `cost-meter`, `listing-suppression`; soft: `parts-rules` (`v_tag_blocks`, to skip keyword-stuffing blocks) and `product-catalogue` (alias view, for the generated stop-list).
