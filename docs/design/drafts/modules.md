...
| `pickup-location` | Resolve where an item really is, from its location field and its text; publish town or area only | `detail-evidence.changed`, `listing-ingest.first-seen` | `.resolved`; `v_areas`; `app.v_pickup_location_areas`; `pointsFor()` | `resolutions`, `calls` | New (owner, 2026-09-24) | MVP (`nabvy/docs/decisions.md:144-153`); AI gated with `parts-ai` (question 12) |
...
| `pickup-routes` | Plan a one-day route through the pickups a user has arranged | Web forms; `account.deleted` | `plan()` only; no views | `pickups` | New (owner, 2026-09-24) | MVP (`nabvy/docs/decisions.md:174`) |
| **Trust and rights** | | | | | | |
...
- **Views:** internal `v_members`, `v_cluster_facts`, `v_listing_copy_facts`, `v_links`, `v_review_queue`, `v_shadow_metrics`; restricted `restricted_accounts` (whether a cluster spans accounts, within one run); user-facing `app.v_copy_advert_flags` (listing_id, towns, span_days, window_days, rule_version).
...
- **Depends on:** `switches`, `alert-router`, `listing-card`, `account`, `spec-match`, `price-drop-watch`, `asking-price-position`, `warning-signs`, `suspected-labels`, `copy-advert`, `prepared-message`, `quote-redaction`, `listing-suppression`, `location`, `pickup-location` (soft).
...
| 1 | `switches` |
| 2 | `product-catalogue`, `apify-gateway`, `account`, `waitlist` |
| 3 | `listing-ingest`, `route-health`, `spend-governor`, `side-discovery`, `product-events`, `cex-adapter`, `scan-recognition`, `usage-ledger`, `marketing-consent`, `retail-comparison` |
...
| 7 | `check-scheduler`, `listing-assessment`, `pickup-location`, `want-manager`, `pasted-link-lookup`, `pickup-routes`, `similar-items`, `inventory` |
| 8 | `photo-review`, `noise-filter`, `demand-signals`, `prepared-message`, `sold-reports`, `ebay-drafts` |
| 9 | `asking-price-index`, `spec-match`, `sold-price-book` |
