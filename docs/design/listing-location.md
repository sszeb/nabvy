# `pickup-location`: where an item really is (condensed)
...
- Precise points, full postcodes, street names and raw mention text stay internal (`pickup_location` schema); only town/area/district reaches a `v_` view or user output.
...
- **`pickup-location`** (existing card, expanded) — resolves listing location from field + text + AI; publishes town/area only. The draft calls this module `listing-location`; coordinator decision (2026-09-24): it keeps its catalogue name `pickup-location`, with the draft's fuller schema and its user-facing view named `app.v_pickup_location`.
...
- **`spec-match`** — distance/inclusion sourced from `app.v_pickup_location` (or the §5.8 fallback), re-evaluates on `pickup-location.changed`, never restricts candidates to the search centre that found them.
...
- **`warning-signs` / `suspected-labels`** — read the location conflict and handover facts as one evidence line, shown to users only while `pickup-location` is `on`.
...
- A shared `@nabvy/listing-text` foundation package is proposed but not built; `pickup-location` and the parts modules keep separate copies of shared helpers until the coordinator creates it and checks they agree.
- Resolved by the coordinator (2026-09-24): the module keeps its catalogue name `pickup-location` (the draft's own working name was `listing-location`); its card now carries the draft's fuller schema (10 tables) and `pointsFor()` stays exported for TypeScript callers alongside the new `app.v_pickup_location` view. See the integration scratchpad's amendment list for the specific deltas applied.
