...
// spend-governor, details-queue, quote-redaction and this module. Built like parts-rules' test
// support. Used as nabvy_pipeline, the role the module runs
...
export async function detailed(
  ...
  const ruled = await runRules(t.db, { listingIds: recordedJob.value.changed })
  if (!ruled.ok) throw new Error(ruled.error.message)
  return ruled.value.ran
}
