...
// apify-gateway, listing-ingest and this module. Used as nabvy_pipeline, the role the module runs as. PGlite has
...
const FILES = [
  ...
  ...['core', 'switches', 'cost-meter', 'apify-gateway', 'listing-ingest', 'run-coverage'].flatMap(
    (m) => sqlIn(`packages/db/migrations/${m}`),
  ),
]
...
export interface TestDatabase {
   ...
   */
  collected(run: RecordedRun, rows?: Json[], options?: CollectedOptions): Promise<number>
  close(): Promise<void>
}

export interface CollectedOptions {
  /** The job's status: `failed` for a run that failed (its rows, if any, are still stored). */
  status?: 'succeeded' | 'failed'
  /** Replaces the RUN_SUMMARY; null stores none. */
...
export async function createTestDatabase(): Promise<TestDatabase> {
  ...
  return {
    ...
    async collected(recorded, rows = recorded.dataset, options = {}) {
      const runSummary = options.runSummary === undefined ? recorded.runSummary : options.runSummary
      const [job] = await asOwner(
        `insert into apify_gateway.jobs (kind, input, status, apify_run_id, result)
         values ('collect', $1::jsonb, $4, $2, $3::jsonb) returning id`,
        [
...
/** Every switch the module reads (and the gateway's and listing-ingest's), on. */
export const ALL_ON = {
  'apify-gateway': 'on',
  apify: 'on',
  pipeline: 'on',
