...
// An in-process Postgres (PGlite) with the real migrations of every module scan() touches: core,
// audit-log, switches, better-auth (account standing), product-catalogue (codes and resolve),
// cost-meter (the ledger) and scan-recognition. PGlite has no pg_trgm, so product-catalogue's
// fuzzy tier is stood in for by an `extensions.similarity` that never matches: a name the
// dictionary does not resolve resolves to nothing here. The full set runs on real Postgres in
// `pnpm db:dry-run` (packages/db/tests/scan-recognition.test.sql).
...
export async function createTestDatabase(): Promise<TestDatabase> {
  ...
  await pg.exec(`create schema if not exists extensions;
    create function extensions.similarity(a text, b text) returns real
