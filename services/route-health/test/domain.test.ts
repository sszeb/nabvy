...
// The same port and tests already exist as groundwork in services/source-adapters (task 1.1); this
// module's own "Depends on" line does not list source-adapters, so the pure logic is reproduced
// here rather than imported (docs/design/modules/_rules.md, rule 1).
...
describe('recommendDetailRoute (Nabvy)', () => {
  ...
  it('never calls no replays low success, even with minAttempts 0 (Nabvy divergence, pinned)', () => {
    ...
    // returns page / low-success with an alert. Nabvy stays on graphql: see domain/route-health.ts.
