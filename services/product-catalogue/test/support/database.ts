...
// An in-process Postgres (PGlite) with the real core, audit-log, switches and product-catalogue
// migrations applied (services/switches/test/support/database.ts). PGlite has no pg_trgm, so the
// fuzzy tier of resolve() is untested here; it runs on real Postgres only, in
// packages/db/tests/product-catalogue.test.sql under `pnpm db:dry-run`.
