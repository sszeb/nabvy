...
// test support. Used as nabvy_pipeline, the role the module runs as; `asApp` runs as nabvy_app. PGlite has
...
/** Every switch the module and the modules it reads need, on. */
export const ALL_ON = {
  'apify-gateway': 'on',
  apify: 'on',
  pipeline: 'on',
 ...
 * the live pipeline does. Returns the job ID.
 */
...
/**
 * A sample user-facing view of listings, built the way rule 5 requires every one to be: it leaves
 * out suppressed listings and returns no rows while listing-suppression is not on. Stands in for
 * `app.v_listing_card` until listing-card exists.
 */
export async function createSampleAppView(t: TestDatabase): Promise<void> {
  await t.sql(`create schema if not exists app`)
  await t.sql(`grant usage on schema app to nabvy_app`)
  await t.sql(`grant usage on schema listing_ingest to nabvy_app`)
  await t.sql(`grant select on listing_ingest.listings to nabvy_app`)
  await t.sql(
    `create view app.v_listing_card_sample with (security_invoker = true) as
  ...
  await t.sql(`grant select on app.v_listing_card_sample to nabvy_app`)
}
