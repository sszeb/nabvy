...
// An in-process Postgres (PGlite) with the real core, switches and travel-cost migrations
// applied, so the tests exercise the actual tables, grants and RLS policies (services/account's
// test harness follows the same shape). PGlite has no PostGIS, pgvector or pg_trgm; nothing here
// uses them, so their `create extension` lines are skipped. The full migration set runs on real
// Postgres in `pnpm db:dry-run` (packages/db/tests/travel-cost.test.sql).
