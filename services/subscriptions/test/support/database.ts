...
// usage-ledger and subscriptions migrations applied, so the tests exercise the actual tables,
// grants, policies and triggers. better-auth is here because Checkout checks the account through
// `better_auth.account_active` (via @nabvy/account); usage-ledger because allowances and top-ups
// are granted through it. PGlite has no PostGIS, pgvector or pg_trgm;
// nothing here uses them, so their `create extension` lines are skipped. The full migration set
// runs on real Postgres in `pnpm db:dry-run` (packages/db/tests/subscriptions.test.sql).
