...
- Exported functions: `record`, `recordRestrictedRead`, `listEntries` (admin screen; newest
  first, filter by actor, target, and `before` for paging), `AuditLogRefused`.
...
- 2026-09-24: `AuditLogEntry` is written in Zod, not derived with `drizzle-zod`: that package is
  not in the repo, and contracts cannot import `@nabvy/db` (db depends on contracts). A test
  compares its keys and types with the Drizzle view declaration.
