...
// An in-process Postgres (PGlite) with the real core and audit-log migrations applied, so the
...
// migration set runs on real Postgres in `pnpm db:dry-run` (packages/db/tests/audit-log.test.sql).
...
export async function createTestDatabase(): Promise<TestDatabase> {
  const pg = new PGlite()
  for (const file of [...migrationFiles('core'), ...migrationFiles('audit-log')]) {
