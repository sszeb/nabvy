...
// apify-gateway, listing-ingest and this module. Built like listing-ingest's test support. Used as nabvy_pipeline, the role the module runs as. PGlite has
...
/** Every switch the module reads (and the gateway's and listing-ingest's, so their views show rows), on. */
export const ALL_ON = {
  'apify-gateway': 'on',
  apify: 'on',
  pipeline: 'on',
...
/**
 * Stores rows as a collected job and lets listing-ingest ingest it, as the live pipeline does
 * before this module's (retried) delivery succeeds. Returns the job ID.
 */
export async function collectedAndIngested(
  t: TestDatabase,
  recorded: RecordedRun,
  rows: Json[] = recorded.dataset,
  kind: 'search' | 'details' = 'search',
): Promise<number> {
  const jobId = await t.collected(recorded, rows)
  const result = await ingest(t.db, { jobId, kind })
