...
- **Functions** from `@nabvy/switches`: `state(q, name)`, `isOn(q, name)`, `gateAllows(q, gate, userId)`, `list(q)`, `set(q, input)` → `{ changed, event }`, `SwitchesRefused`. The caller publishes `event` through `@nabvy/transport` after its transaction commits.

## Tables

`switches.switches`: `name` (primary key, kebab case, unique across kinds), `kind` (`module | provider | gate | flag | global`), `state` (default `off`), `allow_list uuid[]` (gates only), `changed_at`, `changed_by` (null for seeds). Check constraints: `shadow` for modules only; allow-lists for gates only, 1 to 1000 users (an empty list is refused; close a gate by switching it off); `audit-log`, `incidents` and `switches` always `on`. No role may delete a row: a switch is turned off, never removed.
...
| Allow-list size | 1–1000 users | Keeps a gate row small; the alert gate is open to all today | Starting value |
...
- 2026-09-24: `set()` returns the `switches.changed` envelope and the caller publishes it after commit, so no reader sees the event before the new state (review of PR #23). A change to the current value writes nothing and returns the last change's envelope, whose key the transport has already seen, so a publish lost after commit is recovered by repeating the change.
- 2026-09-24: `switches.is_on`'s parameter is named `m` to match the stub in `packages/db/tests/quote-redaction.test.sql`; it can be renamed once that stub is removed.
