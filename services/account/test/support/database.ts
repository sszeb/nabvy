...
// An in-process Postgres (PGlite) with the real core, audit-log, switches, better-auth and
// account migrations applied, so the tests exercise the actual tables, grants and policies,
// including `better_auth.account_active`/`account_restriction` that `isActive()`/`assertActive()`
// call directly. `restrictAccount`/`liftRestriction` need a running Better Auth instance, which
// this harness does not start (services/account/README.md, "Tests"): tests inject a fake for
// them that writes the same `better_auth.user` columns, so `isActive()` proves the real read side
// without this suite also standing up auth's write side. PGlite has no PostGIS, pgvector or
// pg_trgm; nothing here uses them, so their `create extension` lines are skipped. The full
// migration set runs on real Postgres in `pnpm db:dry-run` (packages/db/tests/account.test.sql).
