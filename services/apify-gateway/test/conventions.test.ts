...
describe('who may reach the gateway', () => {
  ...
  it('only this package queues jobs or reads the gateway tables', () => {
    ...
    // listing-ingest's, detail-evidence's and listing-suppression's test support seed collected
    // jobs into the gateway's tables in PGlite (never a live database), so their fixtures read the
    // real v_rows.
    const seeders = [
