...
describe('who may reach the gateway', () => {
  ...
  it('only this package queues jobs or reads the gateway tables', () => {
    ...
    // Readers' test support (listing-ingest, detail-evidence, run-coverage, listing-suppression,
    // relist-merge, copy-advert) seeds collected jobs into the gateway's tables in PGlite (never a
    // live database), so their fixtures read the real views.
    const seeders = [
