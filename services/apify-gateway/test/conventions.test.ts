...
describe('who may reach the gateway', () => {
  ...
  it('only this package queues jobs or reads the gateway tables', () => {
    ...
    // listing-suppression, parts-rules, copy-advert, listing-lifecycle, details-selector,
    // price-drop-watch) seeds collected jobs into the gateway's tables in PGlite (never a live
    // database), so their fixtures read the real views.
    const seeders = [
      ...
      'services/details-selector/test/support/',
