...
// usage-ledger migrations applied, so the tests exercise the actual tables, grants, policies and
// triggers. better-auth is here because chargeUsage() checks the account through
// `better_auth.account_active` (via @nabvy/account). PGlite has no PostGIS, pgvector or pg_trgm;
// nothing here uses them, so their `create extension` lines are skipped. The full migration set
// runs on real Postgres in `pnpm db:dry-run` (packages/db/tests/usage-ledger.test.sql).
...
export async function setSwitch(db: TestDatabase, state: 'off' | 'shadow' | 'on'): Promise<void> {
  await db.sql(
    `insert into switches.switches (name, kind, state) values ('usage-ledger', 'module', $1)
