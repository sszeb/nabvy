...
import { events, type IncidentRow, RecordDeadLetterInput } from '@nabvy/contracts/modules/incidents'
 ...
 * `event_key`). Builds, but does not publish, the `incidents.dead-lettered` event: this module has
 * no transport wiring yet (the Trigger.dev task layer is a later task, `docs/questions.md`); the
 * caller is expected to emit the returned event once that layer exists.
 */
export async function record(db: Queryable, input: RecordDeadLetterInput) {
  const parsed = RecordDeadLetterInput.parse(input)
...
export async function retry(
  db: Queryable,
  incidentId: string,
): Promise<Result<{ envelope: IncidentRow['payload'] }, AppError>> {
