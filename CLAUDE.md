...
- **Model and effort by job** (owner, 2026-09-25 14:55; `docs/decisions.md` "Model by job"). Build and fix sessions run on Opus 5.5 at its default effort, `medium`; the reviewer and the coordinator run on the top model at `medium`; the relay and watchdog Routines run on Haiku. Use the top model only for design, security, money, the pipeline core and adversarial verification. Use Sonnet for workflow readers and critics, UI, docs and CRUD. Use Haiku for pure extraction. Set `model` and `effort` on every workflow agent: `low` for mechanical steps, `medium` by default, `high` for a designer or verifier. Start build sessions with the model their module needs.
...
- **Short sessions: hand off on your own** (owner, 2026-09-24). A long-running session (the coordinator, the reviewer, or a build session that takes on more work) hands off to a fresh one without asking the owner first. The trigger is `get_session` showing `external_metadata.context_usage.used_tokens` above 150,000 (owner, 2026-09-25 14:05, was 300,000), or the session's context having been summarised once, whichever comes first. Hand off at the next point where no edit is half done:
  1. push everything;
